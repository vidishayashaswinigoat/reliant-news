...
export const ChatAssistant: React.FC<ChatAssistantProps> = ({ article }) => {
  ...
  useEffect(() => {
    chatRef.current = createChat(article);
    setMessages([]);
    setError(chatRef.current ? null : "Reliant Assistant is unavailable. API Key may be missing.");
  }, [article]);
    ...
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-800/50">
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        <h3 className="text-lg font-bold flex items-center gap-2">
            <Bot className="text-blue-500" /> Reliant Assistant
        </h3>
        <p className="text-xs text-gray-500 dark:text-gray-400">Your media literacy guide</p>
      </div>
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
            {msg.role === 'model' && (
             ...
             <div className={`max-w-xs md:max-w-sm rounded-2xl p-3 text-sm ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-lg' : 'bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-lg'}`}>
